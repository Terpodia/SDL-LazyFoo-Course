...
int main(int argc, char* args[])
{
    ...
    while(!quit){
        ...
        float average = (framesCount) / (SDL_GetTicks()/1000.f);
        if(average > 2e6) average = 0;

        framerateTextTexture.loadFromRenderedText("Average frames Per Second " + to_string(average), {0, 0, 0, 255});
        framerateTextTexture.render((WIDTH -  framerateTextTexture.getWidth())/2, (HEIGHT - framerateTextTexture.getHeight())/2);
