...
void Dot::handleEvent(SDL_Event& event)
{
    if(event.type == SDL_KEYDOWN && event.key.repeat == 0)
    {
        ...
        {
            case SDLK_UP: yVelocity -= DOT_SPEED;  break;
            case SDLK_DOWN: yVelocity += DOT_SPEED; break;
            case SDLK_LEFT: xVelocity -= DOT_SPEED; break;
            case SDLK_RIGHT: xVelocity += DOT_SPEED; break;
        }     
    }
...
void Dot::move()
{
    xPos += xVelocity;
    yPos += yVelocity;
    if(xPos < 0 || xPos + DOT_WIDTH > WIDTH) xPos -= xVelocity;
    if(yPos < 0 || yPos + DOT_HEIGHT > HEIGHT) yPos -= yVelocity; 
}
