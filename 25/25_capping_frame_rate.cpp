...
const int WIDTH = 640, HEIGHT = 480, SCREEN_FPS = 60, SCREEN_TICKS_PER_FRAME = 1000/SCREEN_FPS;
...
int main(int argc, char* args[])
{
    ...
    while(!quit){
        ...
        float avgFPS = cntFrames/(fpsTimer.getTicks()/1000.f);
        if(avgFPS > 2e6) avgFPS = 0;

        SDL_RenderClear(renderer);
        timeTextTexture.loadFromRenderedText("Average Frames Per Second (With Cap) " + to_string(avgFPS), {0,0,0,255});
        timeTextTexture.render((WIDTH-timeTextTexture.getWidth())/2, (HEIGHT-timeTextTexture.getHeight())/2);
        SDL_RenderPresent(renderer);

        if(capTimer.getTicks() < SCREEN_TICKS_PER_FRAME) SDL_Delay(SCREEN_TICKS_PER_FRAME - capTimer.getTicks());
